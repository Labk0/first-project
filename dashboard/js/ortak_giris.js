const giris_form = document.querySelector("#ortak_giris");
...
giris_form.addEventListener("submit", async (event) => {
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/api/ortaks/login`, {
            method: 'POST',
            credentials: 'include',
            headers: {
        ...
        if (response.ok) {
            alert(`Giriş başarılı! Hoş geldin ${data.first_name} ${data.last_name}.`);
